...
def ema8_vwap_3m(df):
    ...
    if len(d) < 2 or vwap.isna().all():
        return {'bull_cross': False, 'bear_cross': False, 'bull_aligned': False, 'bear_aligned': False, 'ema8': float(e8.iloc[-1]), 'vwap': 0.0, 'text': '3m EMA8/VWAP unavailable'}
...
def score_setup(side, entry_m, higher_m, daily, patterns, momentum, micro3, spread_pct=999.0, trade_type='DAY TRADE'):
    score = 0.0
    reasons = []
    aligned = (side == 'CALL' and higher_m['bull'] and daily['bull']) or (side == 'PUT' and higher_m['bear'] and daily['bear'])
    if aligned:
        score += 2.5
...
def format_alert(ticker, side, score, status, price, pat, pattern_tf, reasons, spread_pct, momentum, micro3, news, options, confirmation, trade_type):
    ...
    return (
        ...
        f"Why: {', '.join(reasons)}\n\n{options['text']}\n\n"
        f"Status: ✅ CONFIRMED — PRICE ACTION VALIDATED"
    )


def format_early_setup(ticker, side, score, price, pat, pattern_tf, reasons, spread_pct, momentum, micro3, confirmation):
    pattern_text = f"{pat.name} ({pattern_tf})" if pat else 'Developing intraday structure'
    trigger = f"{pat.trigger:.2f}" if pat and pat.trigger else 'Waiting for price-action trigger'
    return (
        f"👀 CHIEF EARLY SETUP | {ticker} | DAY TRADE\n"
        f"Observed direction: {side}\nScore: {score}/10\nPrice: {price:.2f}\n"
        f"Pattern: {pattern_text}\n3m Timing: {micro3['text']}\n"
        f"Momentum: {momentum['text']}\nSpread: {spread_pct:.2f}%\n"
        f"Confirmation still needed: {confirmation['text']}\n"
        f"Trigger being watched: {trigger}\n"
        f"Why it is on watch: {', '.join(reasons)}\n\n"
        f"⚠️ EARLY SETUP ONLY — NOT A CONFIRMED ENTRY. No options contract is selected until Chief confirms the setup."
    )
...
def high_confidence_early_setup(side, score, pat, pattern_tf, higher_m, daily, micro3, momentum):
    ...
    daily_ok = daily['bull'] if side == 'CALL' else daily['bear']
    momentum_text = str(momentum.get('text', '')).lower()
    momentum_ok = ('bull' in momentum_text) if side == 'CALL' else ('bear' in momentum_text)
    return bool(micro_ok and htf_ok and (daily_ok or momentum_ok))
...
def _snapshot_resilient(ctx, batch, depth=0):
    ...
    mid = len(working) // 2
    lf, ls = _snapshot_resilient(ctx, working[:mid], depth + 1)
    rf, rs = _snapshot_resilient(ctx, working[mid:], depth + 1)
    return lf + rf, skipped + ls + rs
...
def get_snapshots(ctx, codes):
    ...
    for i in range(0, len(codes), SNAPSHOT_BATCH):
        ...
        if batch_no == 1 or batch_no % 5 == 0 or batch_no == batches:
            print(f'CHIEF stage 1 progress: batch {batch_no}/{batches}, {sum(len(x) for x in frames)} quoted, {len(skipped)} unsupported skipped', flush=True)
        time.sleep(0.35)
    if skipped:
...
def build_deep_scan_list(ctx):
    if not WHOLE_MARKET:
        return [f'US.{x}' for x in CORE_WATCHLIST[:DEEP_CANDIDATES]]
...
def evaluate_trade_mode(ctx, ticker, side, trade_type, price, spread_pct, d3, d5, d15, d60, dd, m15, m60, daily, last_alert, swing_send_count):
    ...
    if trade_type == 'DAY TRADE':
        ...
        if pattern_tf == '5m':
            ...
            entry_df = d15
            if pat:
                reasons.append('15m pattern confirmation')
    else:
        entry_df = d60
        pattern_tf = '1H' if pat else ''

    confirmation = evaluate_confirmation(side=side, df=entry_df, score=score, pattern=pat, min_score=CONFIRMED_SCORE)

    if not confirmation['confirmed']:
        if trade_type == 'DAY TRADE' and signal_window_open() and high_confidence_early_setup(side, score, pat, pattern_tf, higher_m, daily, micro3, momentum):
            early_key = (ticker, side, 'DAY TRADE', 'EARLY', pattern_tf, pat.name if pat else '')
            if time.time() - last_alert.get(early_key, 0) > 900:
                notify(format_early_setup(ticker, side, score, price, pat, pattern_tf, reasons, spread_pct, momentum, micro3, confirmation))
                last_alert[early_key] = time.time()
        return
    ...
    status = 'CONFIRMED'
    key = (ticker, side, trade_type, status, pattern_tf, pat.name if pat else '')
