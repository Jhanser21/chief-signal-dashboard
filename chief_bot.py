                        ...
                        if spread<=MAX_SPREAD_PCT:
                            for side in ('CALL','PUT'):
                                ...
                                if a.get('front_run') and not a.get('confirmed'):
                                    print(f"{ticker} SWING {side}: EARLY {a.get('raw_score',0)}/100 | {a.get('tqe_stage')} | TQE {','.join(a.get('tqe_models',[])) or 'none'} | Daily {a.get('internal_trigger')} -> {a.get('major_trigger')} | 4H {a.get('h4_structure')} {a.get('h4_internal_trigger')} -> {a.get('h4_major_trigger')} | proj RVOL {a.get('projected_rvol',0):.2f}x",flush=True)
