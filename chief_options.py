...
# Chief uses different contracts for the two trading horizons.
OPTION_PROFILES = {
    'DAY TRADE': {
...
def _request(ctx, underlying, side, min_dte, max_dte):
...
def recommend_options(ctx, ticker, side, trade_type='SWING'):
    ...
    try:
        df = _request(ctx, underlying, side, profile['min_dte'], profile['max_dte'])
    except Exception as exc:
        return {'ok': False, 'text': f'⚠️ Option scan unavailable: {exc}', 'picks': []}

    if df is None or getattr(df, 'empty', True):
        return {'ok': False, 'text': f"⚠️ No liquid {trade_type.lower()} contracts matched Chief filters.", 'picks': []}
    ...
    for _, row in df.iterrows():
        ...
        # Long-option theta is negative. Compare decay as % of premium/day.
        ...
        # Swing contracts get slightly more emphasis on theta; day trades on liquidity/spread.
