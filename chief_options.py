...
def _request(ctx, underlying, side, min_dte, max_dte, min_volume=None, min_oi=None):
    ...
    for indicator in (OptIndicator.DELTA, OptIndicator.THETA, OptIndicator.IV, OptIndicator.VOLUME,
                      OptIndicator.OPEN_INTEREST, OptIndicator.BID_ASK_SPREAD, OptIndicator.PRICE, OptIndicator.LEFT_DAY):
        try:
...
def _select_from_df(df, side, trade_type, profile, relaxed=False):
    ...
    for _, row in df.iterrows():
        ...
        dte_fit = max(0.0, 1.0 - abs(dte - target_dte) / span) if dte else 0.5

        if trade_type == 'DAY TRADE':
            quality = delta_fit * 2.6 + theta_fit * 1.8 + spread_fit * 2.4 + volume_fit * 1.6 + oi_fit * 1.0 + dte_fit * 0.6
        else:
            quality = delta_fit * 2.8 + theta_fit * 2.6 + spread_fit * 1.9 + volume_fit * 1.0 + oi_fit * 1.0 + dte_fit * 0.7

        picks.append(OptionPick(code, name, side, strike, dte, expiration, price, delta, theta, iv,
                                volume, oi, spread, spread_pct, theta_pct, quality))
...
def recommend_options(ctx, ticker, side, trade_type='SWING'):
    ...
    if trade_type == 'SWING':
        ...
        attempts += [(3, 30, max(50, MIN_VOLUME // 2), max(250, MIN_OI // 2), True)]

    last_error = None
    for min_dte, max_dte, min_volume, min_oi, relaxed in attempts:
        try:
            df = _request(ctx, underlying, side, min_dte, max_dte, min_volume=min_volume, min_oi=min_oi)
    ...
    if last_error:
        return {'ok': False, 'text': f'⚠️ Option scan unavailable: {last_error}', 'picks': []}
